...
class RevDiffWave(torch.nn.Module):
    ...
    def audio_editing_sample(self, audio):
        ...
        for it in range(self.args.sample_step):
            ...
            total_noise_levels = self.args.t # Sampling noise scale
            if self.args.rand_t: # Decide if randomize sampling noise scale: deault is false
            ...
            #print("hahaha let me observe x_",x_, x_.shape)
            if self.args.use_bm:
            ...
            xs.append(x0)
            noise_list.append(e)
        return torch.cat(xs, dim=0), torch.cat(noise_list, dim=0)

    def forward(self, x):

        output,noise = self.audio_editing_sample(x)
...
class RevDiffWavegggggggggggggg(torch.nn.Module):
    def __init__(self, args, device=None):
        super().__init__()
